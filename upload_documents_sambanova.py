...
print(f"Found {len(chunks)} chunks. Embedding and uploading to SambaNova table...")
...
for i, chunk in enumerate(chunks, 1):
    try:
        ...
        response = supabase.table("documents_sambanova").insert({
            "content": chunk,
            "embedding": embedding
        }).execute()

        if response.data:
            print("✅ Uploaded chunk to documents_sambanova.")
        else:
